// Gemini Prompt Worker (CommonJS)
const { Worker } = require('bullmq');
const { GoogleGenAI } = require("@google/genai");
const { redisConfig, QUEUES } = require('./queue-config.cjs');
...
const processGeminiJob = async (job) => {
    ...
    try {
        // If the producer serialized payloads as JSON strings, parse them here
...
// Create worker (use same plain config object so BullMQ manages its own client)
    ...
    try {
        ...
        if (result && result.response) {
            if (typeof result.response === 'string') {
                snippet = result.response.substring(0, 100);
            } else {
                snippet = JSON.stringify(result.response).substring(0, 100);
            }
        }
        console.log(`✅ Job ${job.id} completed`);
        console.log(`   Result: ${snippet}...`);
    } catch (e) {
        console.log(`✅ Job ${job.id} completed (could not stringify result)`);
    }
});
