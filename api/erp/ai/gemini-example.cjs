// Ví dụ sử dụng: Đẩy job Gemini vào queue (CommonJS)
const { queues, queueEvents, QUEUES } = require('./queue-config.cjs');
...
async function submitGeminiPrompt(prompt, systemPrompt = '', temperature = 0.7, maxTokens = 2048, responseSchema = null, options = {}) {
    ...
    // Serialize payload to a JSON string to ensure Redis receives string args only
    const payload = JSON.stringify(jobData);
    // Use an explicit job name so the signature is (name, data, opts)
    const job = await queues.geminiPrompt.add('gemini-prompt-job', payload, jobOptions);
...
async function waitForGeminiResult(job, timeout = 300000) {
    ...
    const result = await job.waitUntilFinished(queueEvents.geminiPrompt, timeout);
...
async function example2_Callbacks() {
    ...
    submitGeminiPrompt(prompt, 'You are a poet', 0.8)
        .then(job => {
            ...
            job.waitUntilFinished(queueEvents.geminiPrompt)
                .then(result => {
