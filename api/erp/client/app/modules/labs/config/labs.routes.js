/**
 * Created by loc on 09/05/2016.
 */

(function () {
    'use strict';

    angular
        .module('com.module.labs')
        .config(function ($stateProvider) {
            $stateProvider
                .state('app.labs', {
                    abstract: true,
                    url: '/labs',
                    templateUrl: 'modules/labs/views/main.html'
                })
                .state('app.labs.list', {
                    url: '',
                    templateUrl: 'modules/labs/views/list.html',
                    controller: 'LabsListCtrl',
                    resolve: {
                        labs: function(LabService){
                            return LabService.getLabs();
                        }
                    }
            
                })
                .state('app.labs.add', {
                    url: '/add',
                    templateUrl: 'modules/labs/views/form.html',
                    controllerAs: 'ctrl',
                    controller: "LabFormCtrl",
                    resolve: {
                        lab: function () {
                            return {};
                        }
                    }
                })
                .state('app.labs.edit', {
                    url: '/:id/edit',
                    templateUrl: 'modules/labs/views/form.html',
                    controllerAs: 'ctrl',
                    controller: function ($state, PageService, page) {
                        this.editorOptions = {
                            theme: 'monokai',
                            lineWrapping: true,
                            lineNumbers: true,
                            mode: 'markdown'
                        };
                        this.page = page;
                        this.formFields = PageService.getFormFields();
                        this.formOptions = {};
                        this.submit = function () {
                            PageService.upsert(this.page).then(function () {
                                $state.go('^.list');
                            });
                        };
                    },
                    resolve: {
                        page: function ($stateParams, PageService) {
                            return PageService.findById($stateParams.id);
                        }
                    }
                })
                .state('app.labs.view', {
                    url: '/:id',
                    templateUrl: 'modules/labs/views/view.html',
                    controllerAs: 'ctrl',
                    controller: function (page) {
                        this.page = page;
                    },
                    resolve: {
                        page: function ($stateParams, PageService) {
                            return PageService.findById($stateParams.id);
                        }
                    }
                })
                .state('app.labs.delete', {
                    url: '/:id/delete',
                    template: '',
                    controllerAs: 'ctrl',
                    controller: function ($stateParams, $state, PageService) {
                        PageService.delete($stateParams.id, function () {
                            $state.go('^.list');
                        }, function () {
                            $state.go('^.list');
                        });
                    }
                });
        });

})();
