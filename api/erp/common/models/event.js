...
module.exports = function(Event) {
   const updateSitemap = (ctx, next) => {
    ...
    if (ctx.where && !ctx.instance) {
      ...
      shouldTrigger = true;
      console.log('[SITEMAP] Event deleted, triggering sitemap update');
      ...
      if (isNowPublished) {
        shouldTrigger = true;
        console.log('[SITEMAP] Event published, triggering sitemap update');
      }
      // Nếu đang được unpublish - cần check giá trị cũ (chỉ cho update, không phải create)
        ...
        Event.findById(ctx.instance.id, {
          fields: { isPublished: true }
        }).then(previousRecord => {
          if (previousRecord && previousRecord[0].isPublished === true) {
            shouldTrigger = true;
            console.log('[SITEMAP] Event unpublished (was published), triggering sitemap update');
          }
          
          if (shouldTrigger) {
            const clientContext = Event.app.hostname ? Event.app.hostname.split('.')[0] : 
                                (Event.currentDatasource || 'ats');
            
            console.log(`[SITEMAP] Triggering for client: ${clientContext}`);
            debouncedGenerateSitemap(Event.app, 3000, clientContext);
          }
          next();
        }).catch(err => {
  ...
  Event.observe('before save', (ctx, next) => {
    ...
    // Kiểm tra xung đột
    Event.findById(id, { fields: { id: true, updatedAt: true }, include: [] })
      .then((record) => {
        if (!record) {
  ...
  Event.observe('after save', updateSitemap);
  Event.observe('after delete', updateSitemap);
};