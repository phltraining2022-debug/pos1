// ApiService.js
angular.module('app') // Or your actual application module name
  ...
  .factory('ApiService', ['$http', '$q', function ($http, $q) {
    ...
    const service = {
      ...
      login: function (modelName, credentials) {
        const deferred = $q.defer();
        $http.post(getBaseUrl() + modelName + '/login?include=user', credentials)
          .then(response => {
  ...
  .factory('AuthenticatedHttpService', ['$http', '$q', function ($http, $q) {
    ...
    function makeAuthenticatedRequest(method, url, data, additionalHeaders) {
      ...
      return $http(config).then(function (response) {
        ...
        // If 401, clear localStorage to prevent further invalid requests
        if (error.status === 401) {
